...
15. **正式後繼的核對**：用出生紀錄連結的配種紀錄的規則快照（實際種牡馬的系與代數、母馬當時的身分）核對，不用母馬目前的用途（8.4 用途修改從下一次配種生效）。核對三件事：父母與配種紀錄一致；依 8.2 重算的系與代數等於出生紀錄；這次配種是出生紀錄那一代的指定配對（10.3，依 5.2「後繼的系、代數與規則不符」阻止）。偏離規則配到別系種牡馬的產駒（11.6「產駒依實際父馬歸系」）因此會被阻止；零代種牡馬例外配市場母馬只是警告，不阻止。沒有配種紀錄或配到八系以外種牡馬的產駒比照自由配種。
16. **追蹤名**：用母馬的完整馬名（6.4「主要顯示完整馬名」）；正式馬名是空字串或只有空白時視為清空。
